...
class MCellSim:
    ...
    def add_trigger(self, species: Species, mesh_obj: MeshObj = None,  reg: SurfaceRegion = None, folder_name='./') -> None:
        ...
        if mesh_obj:
            ...
            count_str = folder_name + "trigger_data/seed_%04d/%s_%s" % (
                    self._seed, species.name, mesh_obj.name)
            count_list, os, out_times, output = m.create_trigger(
            ...
            count_str = folder_name + "trigger_data/seed_%04d/%s_%s_%s" % (
                    self._seed, species.name, reg.mesh_obj.name, reg.reg_name)
            count_list, os, out_times, output = m.create_trigger(
                self._world, reg_sym, species_sym, count_str, 1e-5)
        else:
            print("TRIGGER is not compatible with WORLD. Nothing added!")
            return
...
def create_trigger(world, where, mol_sym, file_path, step):
